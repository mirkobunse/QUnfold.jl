...
"""
RUN(transformer::Union{AbstractTransformer,FittedTransformer}; τ::Float64=1e-6, n_df::Int=-1, a::Vector{Float64}=Float64[], strategy::Symbol=:softmax, backend::Symbol=:ipopt) =
    _RUN_SVD(transformer, :run, τ, n_df, a, strategy, backend)
...
SVD(transformer::Union{AbstractTransformer,FittedTransformer}; τ::Float64=1e-6, n_df::Int=-1, a::Vector{Float64}=Float64[], strategy=:softmax) =
    _RUN_SVD(transformer, :svd, τ, n_df, a, strategy, :ipopt)
...
_solve(m::_RUN_SVD, M::Matrix{Float64}, q::Vector{Float64}, p_trn::Vector{Float64}, N::Int, b::Vector{Float64}=zeros(length(q))) =
    if m.loss == :run
        solve_maximum_likelihood(M, q, N, b; τ=m.τ, n_df=m.n_df > 0 ? m.n_df : size(M, 2), a=m.a, strategy=m.strategy, backend=m.backend)
