...
for (name, method) in [
        ...
        "RUN (softmax, τ=1e-6, initialization=:training)" => RUN(t; strategy=:softmax, τ=1e-6, initialization=:training),
        "RUN (softmax, τ=1e-6, initialization=:blobel)" => RUN(t; strategy=:softmax, τ=1e-6, initialization=:blobel),
        "RUN (softmax_reg, τ=1e-6, initialization=:uniform)" => RUN(t; strategy=:softmax_reg, τ=1e-6, initialization=:uniform),
        "RUN (softmax_full_reg, τ=1e-6, initialization=:uniform)" => RUN(t; strategy=:softmax_full_reg, τ=1e-6, initialization=:uniform),
